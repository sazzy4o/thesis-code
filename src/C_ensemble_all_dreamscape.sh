...
mkdir -p results_all_metrics_seed_dreamscape
    ...
    do
        ...
        do
            ...
            do
                ...
                do
                    file=./results_all_metrics_seed_dreamscape/ensemble_"$metric"_"$architecture"_probs_"$top_p"_whitespace_nan_"$learning_rate"_"$1"_large_"$seed".json
                    if [ ! -f $file ]; then
                        echo $file
                        sbatch --time=0:10:00 --mem-per-cpu=4000M --mincpus=1 --account def-afyshe-ab \
                            --output ./logs/slurm-%j.out \
                            run_python.sh ./C_ensemble.py $architecture $learning_rate $top_p $metric $1 dreamscape large $seed
                    fi
                done
            done
