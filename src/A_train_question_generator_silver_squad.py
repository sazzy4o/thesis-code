...
elif dataset == 'silver_squad':
    out_folder = Path(models_base_name)/f'{model_dir_name}-silver_squad'/args[1]
    ghanem_et_al_2022_prompt_map = {
        'Belief_states': 'Belief States',
        'Causality': 'Causality',
        'Character_identity': 'Character Identity',
...
def load_dataset_df(path):
    ...
    df = df[df['question_type']!='Unknown']
...
if dataset == 'silver_squad':
    df_train = load_dataset_df(root/'data/squad_silver/train-0.8.jsonl')
    df_validation = load_dataset_df(root/'data/squad_silver/validation-0.8.jsonl')
...
if architecture=='separate_models' or architecture == 'separate_models_t5_lm' or 'soft_attempt' in architecture:
...
if 'soft_attempt' in architecture:
...
if "_soft_prompt_patch" in architecture:
    ...
    with torch.no_grad():
        ...
        for new_toks in new_tokens:
            random_token_sample = [get_random_regular_token_id(5000) for _ in range(100)]
            new_tokens = tokenizer.encode(new_toks)[:-1]
            init_a, new_a = zip(*list(zip(random_token_sample,new_tokens)))
            assert len(init_a) == len(new_a)
            model.shared.weight[list(new_a)] = patch_embeddings[list(init_a)].detach().clone()
