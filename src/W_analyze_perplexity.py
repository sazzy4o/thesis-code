...
#%%
best_map = {}
for row in df_dev.groupby(['architecture','size','seed']).head(1).itertuples():
    best_map[(row.architecture,row.seed)] = f'models-large-{row.seed}-control/{row.architecture}-silver_squad/{row.learning_rate}'
best_map
#%%
