...
args = sys.argv[1:]
# args = [
#     'single_model_with_token_random_token_init-from-silver-soft_attempt_t5_lm', # architecture
#     '5e-5', # learning_rate
#     '0.7', # top_p_value
#     'meteor', # metric
...
if len(args) >= 10:
    ...
    if dataset == 'quail':
        ...
        out_dir = root/'src/results_match_counts_seed_control'
    else:
        model_dir = Path(f'{models_base_name}-{dataset}')/architecture/learning_rate
        out_dir = root/f'src/results_match_counts_seed_control_{dataset}'
...
for path in tqdm(models_dir_list):
    ...
    with open(path/f'predictions_nucleus_{dataset_subset}_{top_p_value}.json') as f:
        q_type = path.name
        current_dict = ensemble_dict.get(q_type,{})
        ...
        for row in data:
            ...
            context_dict['question_types'].append(q_type)
            context_dict['probs'].append(row['probs'])
            if len(context_dict['questions'])==0:
                context_dict['questions'] = list(df[(df['context_id']==row['context_id'])&(df['question_type']==q_type)]['question'])
            ...
            # 'probs': prob.tolist(),
            # }
        ensemble_dict[q_type] = current_dict
#%% Remove duplicates
dedup_model_type = deduplication_method.split('_')[-1]
for q_dict in [ensemble_dict]:
    for gen_dict in q_dict.values():
...
def get_ensemble_scores(ensemble_dict):
    ...
    for q_type, q_type_dict in tqdm(ensemble_dict.items()):
        scores = []
        labels = []
        predictions = []
        contexts = []
        for key,value in tqdm(q_type_dict.items()):
            print(key)
        ...
        # return scores, labels, predictions, contexts
        output_rows.append({
            'mean_score': float(np.mean([np.mean(x) for x in scores])),
            'scores': scores,
...
output_rows = get_ensemble_scores(ensemble_dict)
for row in output_rows:
    print('Score', row['question_type'], row['mean_score'])
