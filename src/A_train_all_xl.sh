...
do
    ...
    do
        # for architecture in single_model single_model_with_token_random_token_init ghanem_et_al_2022_true
        # do
        #     file=./models-$size/$architecture/$learning_rate/model_files/config.json
        #     if [ ! -f "$file" ]; then
        #         sbatch --time=3-00:00:00 --mem-per-cpu=60000M --mincpus=2 \
        #         --account def-afyshe-ab --gres=gpu:a100:4 run_python.sh \
        ...
        do
            ...
            do
                ...
                if [ ! -f "$file" ]; then
                    sbatch --time=3:00:00 --mem-per-cpu=60000M --mincpus=2 \
                    --account def-afyshe-ab --gres=gpu:a100:4 run_python.sh \
