# Modified from: https://github.com/ThilinaRajapakse/simpletransformers/blob/master/simpletransformers/t5/t5_model.py
import json
import logging
import math
import os
...
from lib.t5 import BeamSearchEncoderDecoderOutputWithConfidence, T5ForConditionalGenerationWithConfidence#, T5CConfig
...
MODEL_CLASSES = {
    ...
    "t5c": (T5Config, T5ForConditionalGenerationWithConfidence),
}
...
class T5Model:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        global_step, training_details = self.train(
            ...
            eval_data=eval_data,
            verbose=verbose,
            contexts=train_data.context_id.to_numpy(),
            **kwargs,
        )
    ...
    def train(
        ...
        **kwargs,
        ...
        elif args.batch_strategy == 'context-minibatch':
            assert contexts is not None
            # Based on https://stackoverflow.com/a/68792165/3558475
            index_contexts = np.array(list(enumerate(contexts)))
            index_contexts = index_contexts[index_contexts[:, 1].argsort()]
            shuffled_batches = np.split(index_contexts[:,0].astype(np.int), np.unique(index_contexts[:, 1], return_index=True)[1][1:])
            random.Random(args.manual_seed).shuffle(shuffled_batches)
            chunked_batches = [chunks(batch,4) for batch in shuffled_batches]
            batches = [batch for chunk in chunked_batches for batch in chunk]
            train_dataloader = DataLoader(
                train_dataset,
                batch_sampler=batches,
                num_workers=self.args.dataloader_num_workers,
            )
        elif args.batch_strategy == 'context-fullbatch':
            assert contexts is not None
            # Based on https://stackoverflow.com/a/68792165/3558475
            index_contexts = np.array(list(enumerate(contexts)))
            index_contexts = index_contexts[index_contexts[:, 1].argsort()]
            shuffled_batches = np.split(index_contexts[:,0], np.unique(index_contexts[:, 1], return_index=True)[1][1:])
            random.Random(args.manual_seed).shuffle(shuffled_batches)
            train_dataloader = DataLoader(
                train_dataset,
                batch_sampler=shuffled_batches,
                num_workers=self.args.dataloader_num_workers,
            )
            raise Exception("Partial implementation: will run out of memory with current hardware something like `gradient_accumulation_steps` still needed")
        else:
            raise Exception("Not implemented: unknown batch_strategy")
    ...
    def predict(self, to_predict):
        ...
        for batch in tqdm(
            ...
            outputs=full_outputs.sequences
            # conf_out=full_outputs.confidence
            prob_out=full_outputs.probs
            # print('Conf out:',conf_out.shape)
            # print('Out:',outputs.shape)
            local_probs = []
            for prob, toks in zip(prob_out, outputs):
                local_probs.append(prob[toks!=0].cpu().numpy())
            all_outputs.extend(outputs.cpu().numpy())
            # all_conf_outputs.extend(conf_out.cpu().numpy())
            all_prob_outputs.extend(local_probs if prob_out is not None else [None])
            del prob_out
            # del conf_out
            del outputs
            del full_outputs
        ...
        if self.args.num_return_sequences > 1:
            ...
            return outputs, all_prob_outputs
