...
results_dir = Path(__file__).parent.resolve()/'results_all_metrics_seed_dreamscape'
...
for name, group in grouped_df:
    ...
    for question_type in ['Basic_Story_Elements','Summarizing','Vocabulary','Figurative_Language','Inferring','Close_Reading','Predicting','Character_Traits','Visualizing']:
        file_path=f"./models-{size}-{seed}-dreamscape/{architecture}/{learning_rate}/{question_type.replace('_',' ')}/predictions_nucleus_{testset_name}_{top_p}.json"
        print(
            ...
            f"{architecture} {learning_rate} {question_type} {top_p} {testset_name} dreamscape {size} {seed}"
            "; fi"
        )
# %%
metrics = [
...
for metric in metrics:
    ...
    for name, group in grouped_df:
        ...
        # metric = top['metric']
        file_path = f"./results_all_metrics_seed_dreamscape/ensemble_{metric}_{architecture}_probs_{top_p}_{deduplication_method}_{deduplication_cutoff}_{learning_rate}_{testset_name}_{size}_{seed}.json"
        print(
            f"if [ ! -f {file_path} ]; then "
            "sbatch --time=0:20:00 --mem-per-cpu=4000M --mincpus=1 --account def-afyshe-ab --output ./logs/slurm-%j.out "
            f"run_python.sh ./C_ensemble.py {architecture} {learning_rate} {top_p} {metric} {testset_name} dreamscape {size} {seed}"
            "; fi"
        )
# ./C_ensemble.py $architecture $learning_rate $top_p $metric whitespace nan $1 dreamscape models results_all_metrics_dups
# %%
grouped_df = df_all.groupby(['architecture','metric','size','seed'])
testset_name = "test"
for name, group in grouped_df:
